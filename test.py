...
st.markdown(
    ...
    .app-title {
        ...
        gap:12px;
        padding:16px 20px;
        background: linear-gradient(90deg, rgba(31,119,180,0.08), rgba(255,127,14,0.03));
    ...
    .card {
        ...
        border: 1px solid rgba(31,119,180,0.06);
    }
    .muted {
        color: var(--muted);
        font-size:0.95rem;
...
st.markdown(
    """
    <div class="app-title">
      <img src="https://img.icons8.com/ios/28/1f77b4/factory.png" alt="factory" />
      <h1>Polymer Production Scheduler</h1>
      <div style="margin-left:auto;" class="muted">v1.0 • Refactored</div>
    </div>
    """
    , unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown('Use the form to configure parameters and upload your Excel file.')
    with st.form('params_form'):
        ...
        continuity_bonus = st.number_input(
            ...
            min_value=0,
            value=1,
            help='Bonus for continuing the same grade (reduces transitions)'
        )
        submit_params = st.form_submit_button('Save inputs')
    if submit_params:
        st.success('Inputs saved — switch to the Optimization tab to run the model.')
    st.markdown('---')
    st.caption('Tip: keep large file uploads ready before starting the run.')


tabs = st.tabs(["📋 Data Preview", "⚙️ Optimization", "📈 Results"]) 
tab_preview, tab_opt, tab_results = tabs
