...
# ---------------- SOLVER LOGIC — DO NOT MODIFY ----------------
# (These functions/classes are part of data-processing / solver utilities.
#  They are left exactly as in the original script to preserve behavior.)
...
# ---------------- END SOLVER LOGIC BLOCK (DO NOT MODIFY) ----------------


# ---------------- UI & THEME (Material Minimalist) ----------------
st.set_page_config(
...
# Material Minimalist CSS — desktop-first, subtle elevation, neutral palette + accent
st.markdown("""
<style>
/* Layout reset */
...
.mm-card {
    ...
    border-radius: 10px;
    padding: 20px;
    box-shadow: 0 4px 10px rgba(28, 37, 52, 0.06);
...
.result-card {
    ...
    background: #ffffff;
    border: 1px solid #eef1f4;
    box-shadow: 0 4px 14px rgba(28, 37, 52, 0.04);
    margin-bottom: 18px;
}
...
.metric .label { font-size: 12px; opacity: 0.9; text-transform: uppercase; letter-spacing: 0.6px; }

/* Tabs small tweak */
.stTabs [data-baseweb="tab-list"] { background: transparent; padding: 6px; gap: 8px; }
...
# Header
st.markdown("""
<div class="app-header mm-card">
    <div style="display:flex; flex-direction:column;">
        <div style="display:flex; align-items:center; gap:12px;">
            <div class="accent-pill">🏭 Polymer Scheduler</div>
            <div style="display:flex; flex-direction:column;">
                <h1 style="margin-bottom:0;">Polymer Production Scheduler</h1>
                <div class="helper">Multi-plant constrained production scheduling • Desktop-first Material Minimalist UI</div>
            </div>
        </div>
    </div>
</div>
...
# Top-level layout: Upload (left), Parameters (middle), Summary+CTA (right)
col_upload, col_params, col_summary = st.columns([1.2, 1, 0.9])
...
with col_params:
    ...
    buffer_days = st.number_input("Buffer days", min_value=0, max_value=7, value=3, help="Additional days for planning buffer")
    st.markdown("---")
    st.markdown("<strong>Objective weights</strong>", unsafe_allow_html=True)
    stockout_penalty = st.number_input("Stockout penalty", min_value=1, value=10, help="Penalty weight for stockouts in objective function")
    transition_penalty = st.number_input("Transition penalty", min_value=1, value=10, help="Penalty weight for production line transitions")
    continuity_bonus = st.number_input("Continuity bonus", min_value=0, value=1, help="Bonus for continuing the same grade (negative penalty)")
    st.markdown("</div>", unsafe_allow_html=True)
...
with col_summary:
    ...
    st.markdown("---")
    st.markdown("<strong>Parameters</strong>", unsafe_allow_html=True)
    st.markdown(f"- Time limit: **{time_limit_min}** minutes  \n- Buffer days: **{buffer_days}**  \n- Stockout penalty: **{stockout_penalty}**  \n- Transition penalty: **{transition_penalty}**  \n- Continuity bonus: **{continuity_bonus}**")
    st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)

    run_button = st.button("🎯 Generate Optimized Results", use_container_width=True)
    st.markdown("</div>", unsafe_allow_html=True)

# Keep a place for outputs (so the CTA is above the fold)
output_container = st.container()

# When user clicks Run, execute the original processing & solver code (preserved verbatim).
if uploaded_file and run_button:
    # Write uploaded_file to a BytesIO and then execute the original processing & solver logic
    # ---------------- SOLVER LOGIC — DO NOT MODIFY ----------------
    # (The large block below is the original processing, optimization, and plotting code.
    #  I moved it here unchanged — this block must remain exactly as provided.)
    try:
...
# If user hasn't uploaded a file: show the quick-start / sample download (kept simple)
if not uploaded_file:
    st.markdown("<div class='mm-card'>", unsafe_allow_html=True)
    st.markdown("### 🚀 Quick Start")
    st.markdown("1. Download the sample template below\n2. Upload your Excel (.xlsx) using the Upload card\n3. Set parameters & click **Generate Optimized Results**")
    # Attempt to load sample workbook from same folder — reuse solver helper
    try:
        ...
        st.info("Sample template not found on disk.")
    st.markdown("</div>", unsafe_allow_html=True)

# Footer
st.markdown("---")
st.markdown("<div style='text-align:center; color:#6c727a; font-size:13px'>Polymer Production Scheduler • Material Minimalist UI • Streamlit</div>", unsafe_allow_html=True)
