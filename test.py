...
# ============================
# MATERIAL MINIMALIST STYLING
# ============================
st.markdown("""
<style>
    /* Material Minimalist Base Styling */
    .main-header {
        font-size: 2.5rem;
        font-weight: 700;
        color: #2c3e50;
...
<div class="subtitle">
    Multi-Plant Optimization with Shutdown Management • Material Minimalist Interface
</div>
""", unsafe_allow_html=True)
...
with st.container():
    ...
    with col1:
        ...
        if uploaded_file:
            ...
            try:
                ...
                st.info(f"""
                **Data Summary:**
                - **Plants:** {len(plant_df)} production lines
                - **Grades:** {len([col for col in demand_df.columns if col != demand_df.columns[0]])} material types
                - **Planning Period:** {len(demand_df)} days
                """)
    ...
    with col2:
        ...
        st.download_button(
            ...
            help="Get started with our pre-formatted template"
        )
...
# ============================
if uploaded_file:
    st.markdown('<div class="section-header">⚙️ Optimization Parameters</div>', unsafe_allow_html=True)
...
# ============================
# OUTPUT SECTION (ORIGINAL LOGIC PRESERVED)
# ============================
if uploaded_file and st.session_state.current_step >= 2:
    st.markdown("---")
    st.markdown('<div class="section-header">📈 Optimization Results</div>', unsafe_allow_html=True)
    
    # --- ORIGINAL SOLVER LOGIC (PRESERVED EXACTLY) ---
    ...
    if st.session_state.current_step == 2:
        ...
        # Display results in Material cards
        if solution_callback.num_solutions() > 0:
            ...
            with col1:
                st.markdown(f"""
                    <div class="metric-card">
                        ...
                        <div style="font-size: 0.75rem; color: #666; margin-top: 0.25rem;">Lower is Better</div>
                    </div>
                """, unsafe_allow_html=True)
            with col2:
                st.markdown(f"""
            ...
            with tab1:
                # Production visualization (original Plotly code preserved)
...
st.markdown("---")
st.markdown(
    "<div style='text-align: center; color: gray; font-size: 0.9rem;'>Polymer Production Scheduler • Material Minimalist Design • Multi-Plant Optimization</div>",
    unsafe_allow_html=True
)
